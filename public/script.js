...
async function checkSession(retries = 3) {
  try {
    ...
    if (deviceNameDisplay) {
      deviceNameDisplay.textContent = `Device: ${data.deviceName || "Unknown"}`;
    }
    ...
    if (!isLoggedIn) {
      if (retries > 0) {
        console.log(`⏳ Session belum ready, retry... (${retries})`);
        await new Promise((r) => setTimeout(r, 500));
        return checkSession(retries - 1);
      }
      console.warn("❌ Session invalid, redirect ke login");
      window.location.href = "/";
      return;
    }
...
async function loadWorkers() {
  try {
    const response = await fetch("/workers", { credentials: "same-origin" });
...
function renderDeviceList() {
  ...
  deviceList.innerHTML = sortedWorkers
    ...
    <div class="device-item ${
      selectedWorkers.some((w) => w.id === worker.id) ? "selected" : ""
    }" data-id="${worker.id}">
      <div class="device-header">
        <span class="device-name">
          ...
          ${
            worker.discovered
              ? ' <small style="color:#999;">(auto)</small>'
              : ""
          }
          ${
            worker.manual
              ? ' <small style="color:#4285f4;">(manual)</small>'
              : ""
          }
        </span>
        <span class="device-status ${worker.status}">${
